...
class Http
{
	...
	public function parseXML($body)
	{
	    $head = '';
		if ($body instanceof ResponseInterface) {
		    $head = $body->getHeader('Content-Type');
			$body = $body->getBody();
		}
		...
		$this->checkAndThrow($body, $head);
		try {
			$contents = $this->parseNamespaceXml($body);
	...
	protected function checkAndThrow($contents, $head)
	{
        $exist = $this->isText($contents, $head);
        if ($exist) {
            throw new TeleComException($contents);
        }
        switch ($contents)
		{
			case '-1' :
				throw new TeleComException('用户名或密码错误');
				break;
			case '-2' :
				throw new TeleComException('参数错误');
				break;
			case '-3' :
				throw new TeleComException('sign参数错误');
				break;
            case '-4' :
                throw new TeleComException('iccid号码错误或不存在');
                break;
			default :
				return true;
		}
	 ...
	 * @return string
	 */
	public function fuckTheWeChatInvalidJSON($invalidJSON)
	{
     ...
     * @return bool
     */
    public function isText($content, $head)
    {
        ...
        if ($head && str_contains($head[0], 'text/html')) {
            $exist = preg_match_all("/^</", $content);
            if (!$exist) {
                return true;
            } else {
                return false;
